...
import { AppLanguage, RoomBooking, MeetingRoom, UserRole } from "../types";
...
export default function Dashboard({ bookings, rooms, language, setActiveTab, userRole = "user" }: DashboardProps) {
  ...
  return (
    <div id="dashboard-view" className="font-sans pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 backdrop-blur-md border border-white/20 text-amber-300 text-[11px] font-extrabold uppercase tracking-wider shadow-xs">
              ...
              <span>
                {isAdmin 
                  ? (isLao ? "ສູນລວມຂໍ້ມູນ ແລະ ການວິເຄາະສະຖິຕິ" : "Analytical Center & Live Statistics")
                  : (isLao ? "ຕາຕະລາງ ແລະ ລະບົບປະຕິທິນການຈອງ" : "Schedule & Calendar Overview")}
              </span>
            </div>
            ...
            <p className="text-xs sm:text-sm text-indigo-100 font-medium leading-relaxed">
              {isLao 
                ? (isAdmin 
                    ...
                    : "ຕິດຕາມກອງປະຊຸມປະຈຳວັນ ແລະ ຕາຕະລາງການນຳໃຊ້ຫ້ອງປະຊຸມຜ່ານລະບົບປະຕິທິນ.")
                : (isAdmin
                    ...
                    : "View today's scheduled meetings and check room availability via the interactive calendar.")}
            </p>
          </div>
        </div>
      </div>

      {/* Overview Stats Bento (Admin Only) */}
      {isAdmin && (
      ...
      {isAdmin ? (
        ...
        /* Full width Today's Meetings Panel for General Users */
        <div id="today-meetings-panel" className="bg-white dark:bg-[#1e293b] p-6 rounded-3xl shadow-xs border border-slate-100 dark:border-white/5 border-t-4 border-t-violet-500 flex flex-col">
          <div className="bg-gradient-to-r from-violet-600 via-purple-600 to-fuchsia-600 p-4 rounded-2xl shadow-md text-white flex items-center justify-between gap-3 mb-4 shrink-0">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-xl bg-white/20 backdrop-blur-md flex items-center justify-center shadow-sm shrink-0">
                <Calendar className="w-5 h-5 text-white animate-bounce" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {todayMeetings.length === 0 ? (
              ...
              todayMeetings.map((meeting) => (
                <div key={meeting.id} className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-white/5 relative overflow-hidden group hover:border-indigo-500/20 transition-all duration-200">
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-xs bg-blue-500/15 text-blue-600 dark:text-blue-400 font-bold px-2.5 py-0.5 rounded-full">
                      {meeting.startTime} - {meeting.endTime}
                    </span>
                  ...
                  <h4 className="font-bold text-sm line-clamp-1 mb-1 text-slate-800 dark:text-slate-100">
                    {meeting.title}
                  </h4>
      ...
      {/* Calendar Section (Available for both Admin and General Users) */}
      <DashboardCalendar 
        bookings={bookings} 
        rooms={rooms} 
        language={language} 
