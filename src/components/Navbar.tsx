  ...
  return (
    <header id="navbar-header" className="h-28 w-full flex items-center justify-between px-4 sm:px-6 md:px-8 border-b-2 border-amber-400/50 bg-gradient-to-r from-slate-950 via-indigo-950 to-slate-950 text-white sticky top-0 z-30 shadow-[0_10px_35px_rgba(0,0,0,0.65)] backdrop-blur-3xl relative overflow-visible">
      ...
      <div id="navbar-right" className="flex items-center justify-end gap-2 sm:gap-3 min-w-0">
        ...
        {/* In-App Notification Email Mail Button (ข້າງກະດິ່ງການແຈ້ງເຕືອນ) */}
        <div className="relative">
          <button
            ...
            title={isLao ? "ເມວແຈ້ງເຕືອນລະບົບ (Email Notifications)" : "Email Notifications Outbox"}
          >
          ...
          <AnimatePresence>
            {showEmailLogs && (
              <motion.div
                ...
                className="absolute right-0 mt-3 w-80 sm:w-96 bg-white dark:bg-[#1e293b] rounded-3xl shadow-2xl p-5 border border-slate-100 dark:border-white/5 overflow-hidden z-40 max-h-[480px] flex flex-col font-sans"
              >
                <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-white/5">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-sm text-slate-800 dark:text-slate-100">
                      {isLao ? "ເມວແຈ້ງເຕືອນລະບົບ" : "Email Outbox"}
                    </h3>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 mt-4 pr-1">
                  {emailLogs.length === 0 ? (
                    <div className="text-center py-8 text-xs text-slate-400 font-medium">
                      {isLao ? "ບໍ່ມີບັນທຶກເມວແຈ້ງເຕືອນ" : "No email notification logs found"}
                    </div>
                  ) : (
                    emailLogs.map((log) => (
                      <div 
                      ...
                      >
                        ...
                        <div 
                          className="text-[10px] opacity-75 line-clamp-2 bg-slate-100 dark:bg-slate-900/60 p-2 rounded-xl font-mono border border-slate-200/50 dark:border-white/5"
                          dangerouslySetInnerHTML={{ __html: log.body }}
                        />
                        <div className="text-[9px] text-indigo-500 font-extrabold tracking-wider mt-2 uppercase flex items-center gap-1 justify-end">
      ...
      {/* Selected Email Log Reader Modal Portal */}
      {typeof document !== "undefined" && createPortal(
        <AnimatePresence>
          {selectedEmail && (
            <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 bg-slate-950/80 backdrop-blur-md">
              ...
              >
                ...
                <div className="flex items-center gap-3 pr-10 border-b border-slate-100 dark:border-white/10 pb-3.5">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-emerald-600 dark:text-emerald-400 font-extrabold uppercase tracking-wider mt-0.5">
                      {isLao ? "ຈຳລອງການສົ່ງເມວລະບົບ (Simulated Email Outbox)" : "Simulated Email Outbox"}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-900/60 rounded-2xl p-3.5 border border-slate-200/80 dark:border-white/10 text-xs font-medium space-y-1.5 text-slate-700 dark:text-slate-300">
                  ...
                  <div className="flex justify-between">
                    <span className="opacity-70 font-bold">{isLao ? "ລະຫັດ ເມວ:" : "Mail ID:"}</span>
